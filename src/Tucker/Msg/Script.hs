...
instance Default ScriptState where
    def = ScriptState { eval_stack = [] }
...
popS = do
    ...
    if null stack then failT "poping on an empty stack"
    else do
...
runEval :: [ScriptOp] -> Either TCKRError ScriptState
runEval ops =
    execStateT (evalS ops) def `catchError` Left
